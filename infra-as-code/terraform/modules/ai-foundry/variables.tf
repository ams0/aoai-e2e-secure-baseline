...
variable "agent_subnet_id" {
  ...
  validation {
    condition     = length(var.agent_subnet_id) > 0
    error_message = "Agent subnet resource ID must be provided."
  }
}
...
variable "ai_foundry_portal_user_principal_id" {
  ...
  validation {
    condition     = length(var.ai_foundry_portal_user_principal_id) == 36
